...
public class HoneyStickyPistonTileEntityRenderer extends TileEntityRenderer<HoneyStickyPistonTileEntity> {
   ...
   public void render(HoneyStickyPistonTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
      ...
      if (world != null) {
         BlockPos blockpos = tileEntityIn.getBlockPos().relative(tileEntityIn.getMovementDirection().getOpposite());
         BlockState blockstate = tileEntityIn.getMovedState();
         if (!blockstate.isAir()) {
            BlockModelRenderer.enableCaching();
            matrixStackIn.pushPose();
            matrixStackIn.translate((double)tileEntityIn.getXOff(partialTicks), (double)tileEntityIn.getYOff(partialTicks), (double)tileEntityIn.getZOff(partialTicks));
            if (blockstate.is(ModBlocks.HONEY_STICKY_PISTON_HEAD.get()) && tileEntityIn.getProgress(partialTicks) <= 4.0F) {
            ...
            } else if (tileEntityIn.isSourcePiston() && !tileEntityIn.isExtending()) {
               ...
               BlockPos blockpos1 = blockpos.relative(tileEntityIn.getMovementDirection());
